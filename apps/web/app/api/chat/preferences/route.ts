...
async function handleMock(
  ...
  const mockReplies: Record<number, { text: string; complete: boolean; prefs?: any }> = {
    1: {
      text: `好嘞！先定个调——这次聚会想怎么搞？🐦
...
D. 还没想好，交给海鸥参谋`,
    ...
    2: {
      text: `收到！那口味方面有啥雷区不？不吃辣？不吃香菜？素食？还是百无禁忌？跟我说说 🐦`,
      complete: false,
    },
    3: {
      text: `记住了！人觉预算呢？
...
C. 不差钱（随便花）`,
    ...
    4: {
      text: `了解！最后问一下，你们在哪个片区？方便我帮你算距离~ 比如海岸城、万象天地、科技园…

或者直接「深圳」也行，海鸥帮你默认 🐦`,
      complete: false,
    },
    5: {
      text: `齐活了！你的偏好海鸥都记下了 🐦 去 AI 推荐标签页生成方案吧~
---PREFERENCES---
{"eventMode":"meal_only","vibe":"随便","dietary":["不吃辣"],"budget":"$$","location":"海岸城","activityType":[],"hardConstraints":[],"decisionStyle":"follow_group","hometownHint":null,"handoffSummary":"用户聚会模式为只吃饭，不吃辣，预算中档，位置海岸城，偏好收集完整"}`,
      complete: true,
      prefs: {
        eventMode: 'meal_only',
        vibe: '随便',
        ...
        activityType: [],
        hardConstraints: [],
        decisionStyle: 'follow_group',
        hometownHint: null,
        handoffSummary: '用户聚会模式为只吃饭，不吃辣，预算中档，位置海岸城，偏好收集完整',
      },
    },
  };

  const entry = mockReplies[round];
  if (!entry) {
    return NextResponse.json({
      reply: '偏好已收齐！去 AI 推荐标签页生成方案吧 🐦',
      complete: true,
    });
  }

  if (entry.complete && entry.prefs) {
    try {
      ensureDemoEvent();
      const prefs = entry.prefs;
      const dietary = Array.isArray(prefs.dietary) ? prefs.dietary : [];
      const hardConstraints = Array.isArray(prefs.hardConstraints) ? prefs.hardConstraints : [];
      const allConstraints = [...new Set([...dietary, ...hardConstraints])];
      upsertPreferences('demo-invitation', 'demo-event', {
        dietary: allConstraints,
        vibe_pref: prefs.vibe || null,
